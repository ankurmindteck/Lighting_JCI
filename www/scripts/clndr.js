...
(function (factory) {
  ...
  var clndrTemplate = "<div class='clndr-controls'>" +
    ...
    "<% for(var i = 0; i < numberOfRows; i++){ %>" +
      ...
      "</div><div class='indicator<%= days[d].typeClasses %>'></td>" +
      "<% } %>" +
      "</tr>" +
    "<% } %>" +
    "</tbody>" +
  ...
  Clndr.prototype.createDayObject = function(day, monthEvents) {
    ...
    if(eventsToday.length) {
	  ...
	  if(eventsToday[0].type=="Range")
		extraTypeClasses += (" " + this.options.typeClasses.heat);
	  if(eventsToday[0].type=="Repeat")
		extraTypeClasses += (" " + this.options.typeClasses.cool);
	  /*if(eventsToday[0].title=="Single")
		extraTypeClasses += (" " + this.options.typeClasses.auto);*/
    }
    if(!this.options.lengthOfTime.days) {

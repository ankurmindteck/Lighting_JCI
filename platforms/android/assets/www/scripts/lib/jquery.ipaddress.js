	...
	$.fn.ipaddress = function() {
		return $(this).each(function(){
			...
			if (!$this.hasClass('ip-enabled')) {
				...
				for (var i = 0; i <=3; i++) {
					octets.push('<input type="number" data-role="none" data-mini="true" class="ip_octet" id="' + id_prefix + '_octet_' + (i+1) + '" maxlength="3" pattern="[0-9]*" value="' + ip_value[i] + '" />');
				}
				var octet_html = octets.join('.');
				$this.after($('<div class="ip_container"/>').html(octet_html));
			...
			$('div.ip_container').bind('focusin', function(){
				$('#routerConfigDiv').slideUp("fast");
				$('#routerPlus').show();
			});
			$('div.ip_container').bind('focusout', function(){
				$('#routerConfigDiv').slideDown("fast");
				$('#routerPlus').hide();
			});
